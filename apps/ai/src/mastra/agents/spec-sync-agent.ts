...
export const specSyncAgent = new Agent({
  ...
  description:
    'Compare vehicle configurations, discover capabilities and find sourced reviews, articles and videos.',
  model: gemini,
...
Use discoverVehicleContent when asked to find external articles, blogs, social posts or YouTube videos. Returned links are discovery results, not reviewed evidence. Do not quote them, claim transcript access, or claim their content was ingested. Vehicle specifications can be ingested through the dedicated curator workflow. For ingestion requests, use discoverVehicleSpecificationSources to find official Brazilian manufacturer pages or PDFs, then prepareVehicleIngestion after resolving source, brand, model, exact configuration name and model year. Return the prepared form link. Do not claim a job started or data was saved: the curator starts extraction in that form and reviews the resulting draft before publishing. Never ask for, accept, or send curator credentials through chat. Reviews and videos are not supported by specification ingestion. If review evidence is empty, state that and offer or perform external discovery when requested.

...
import { attributeSchema } from './contracts';
...
export async function retrieveReviewEvidence(
  input: Record<string, unknown>,
  signal?: AbortSignal,
) {
  const result = await retrieveReviews(input, signal);
  signal?.throwIfAborted();
  return reviewEvidenceResultSchema.parse({ ...result, kind: 'reviews' });
}

/** The requested operation declares the result schema before retrieval. */
export function retrieveTypedKnowledge(
  operation: 'concepts',
  input: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<z.infer<typeof conceptResultSchema>>;
export function retrieveTypedKnowledge(
  operation: 'capabilities',
  input: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<z.infer<typeof capabilityResultSchema>>;
export function retrieveTypedKnowledge(
...
export function retrieveTypedKnowledge(
  ...
  input: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<z.infer<typeof evidenceExcerptResultSchema>>;
export async function retrieveTypedKnowledge(
  operation: 'concepts' | 'capabilities' | 'related-reviews' | 'evidence',
  input: Record<string, unknown>,
  signal?: AbortSignal,
) {
  const result = await retrieveGraph(operation, input, signal);
  signal?.throwIfAborted();
  switch (operation) {
    case 'concepts':
      return conceptResultSchema.parse({ ...result, kind: 'concepts' });
    case 'capabilities':
      return capabilityResultSchema.parse({ ...result, kind: 'capabilities' });
    case 'related-reviews':
      return reviewEvidenceResultSchema.parse({ ...result, kind: 'reviews' });
    case 'evidence':
      return evidenceExcerptResultSchema.parse({ ...result, kind: 'excerpts' });
  }
}

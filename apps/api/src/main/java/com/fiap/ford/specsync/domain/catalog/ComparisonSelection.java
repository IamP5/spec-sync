...
public record ComparisonSelection(List<UUID> configurationIds, List<String> attributes)
        implements ValueObject, AssertionConcern {
    public ComparisonSelection {
        assertConditionTrue(
                configurationIds != null && configurationIds.size() >= 2 && configurationIds.size() <= 5,
                "configurationIds",
                "Choose between 2 and 5 configurations.");
        assertConditionTrue(

...
@Component({
  ...
  // Only a result with items earns a card. A failure, an empty result or a
  // pending call is one quiet line in the reply column, like the tool notes
  // the transcript renders; the raw tool result stays in the activity view.
  template: `
    @if (quiet(); as line) {
      <div
      ...
      <z-card class="rounded-xl p-4 text-sm shadow-none">
        ...
        @if (specificationDiscovery()) {
          <p class="my-2 text-xs text-muted-foreground" i18n>
  ...
  implements ToolRenderer<Record<string, unknown>>
{
  ...
  /** The one-line outcome when there is no item to lay out; undefined when the card renders items. */
  protected readonly quiet = computed(() => {
    if (this.internal() && !this.failed()) return undefined;
    const data = this.result();
    if (!data)
      return this.toolCall().status === 'complete'
        ? $localize`No valid result returned.`
        : $localize`Retrieving information…`;
    if (data.items?.length) return undefined;
    if (this.failed())
      return $localize`Unavailable right now. Ask the assistant to try again.`;
    if (this.emptyContent()) return this.noExternalLinks;
    return data.message || $localize`No matching results.`;
  });

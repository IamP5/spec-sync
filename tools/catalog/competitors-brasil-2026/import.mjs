...
const allModels = () =>
  manifest.brands.flatMap((brand) =>
    brand.models.map((model) => ({ ...model, brand: brand.name })),
  );
...
function validate() {
  ...
  assert.ok(manifest.brands.length > 0);

  const sources = new Map();
  for (const source of manifest.sources) {
    assert.match(source.sha256, /^[0-9a-f]{64}$/);
    assert.ok(['PRIMARY_SOURCE', 'CURATED_NOTES'].includes(source.provenance));
    assert.ok(source.upstreamUrls.every((url) => url.startsWith('https://')));
    assert.ok(!sources.has(source.key), `Duplicate source: ${source.key}`);
  ...
  for (const attribute of manifest.attributes) {
    ...
    assert.ok(
      !attributes.has(attribute.code),
      `Duplicate attribute: ${attribute.code}`,
    );
  ...
  for (const model of allModels()) {
    assert.ok(model.competesWithFord.length > 0);
    for (const configuration of model.configurations) {
      const identity = `${model.brand}/${model.name}/${configuration.name}/${configuration.market}/${configuration.modelYear}`;
      assert.ok(
  ...
  assert.equal(manifest.brands.length, manifest.expected.brandCount);
  assert.equal(allModels().length, manifest.expected.modelCount);
  assert.equal(identities.size, manifest.expected.configurationCount);
}

const sourceLookup = (key) => {
  const source = manifest.sources.find((item) => item.key === key);
  return `(
    SELECT id FROM catalog.source_revision
    WHERE path=${literal(source.path)} AND sha256=${literal(source.sha256)}
  )`;
};
const evidenceId = (evidence) =>
  uuid(`evidence:${evidence.source}:${evidence.locator}:${evidence.excerpt}`);
const configurationLookup = (brand, model, configuration) => `(
  ...
  WHERE b.name = ${literal(brand)} AND m.name = ${literal(model)}
...
function sourceSql(source) {
  ...
  SELECT ${literal(uuid(`source:${source.path}:${source.sha256}`))}::uuid,
    ${literal(source.path)}, ${literal(source.sha256)}, ${literal(source.title)},
    ${literal(source.provenance)}, ${json(source.upstreamUrls)},
    ${literal(source.capturedOn)}::date, ${nullableLiteral(source.publishedOn)}::date
  WHERE (SELECT run FROM _competitor_import_guard)
...
function evidenceSql(evidence) {
  ...
  SELECT ${literal(evidenceId(evidence))}::uuid, ${sourceLookup(evidence.source)},
    1, 1, ${literal(evidence.excerpt)}, ${literal(evidence.locator)}
  WHERE (SELECT run FROM _competitor_import_guard)
...
function attributeSql(attribute) {
  ...
  SELECT ${literal(uuid(`attribute:${attribute.code}`))}::uuid, ${literal(attribute.code)},
    ${literal(attribute.label)}, ${literal(attribute.description)},
    ${literal(attribute.valueType)}, ${nullableLiteral(attribute.unit)}
  WHERE (SELECT run FROM _competitor_import_guard)
...
function configurationSql(model, configuration) {
  ...
  const lookup = configurationLookup(model.brand, model.name, configuration);
  const deterministicId = uuid(
    `configuration:${model.brand}:${model.name}:${configuration.name}:${configuration.market}:${configuration.modelYear}`,
  );
  const statements = [
    ...
    SELECT ${literal(deterministicId)}::uuid, m.id, ${literal(configuration.name)},
      ${literal(configuration.market)}, ${configuration.modelYear},
      'RESOLVED_FROM_PRIMARY_SOURCE', ${literal(evidenceId(identity))}::uuid,
      ${literal(identity.note)}
    FROM catalog.vehicle_model m
    JOIN catalog.brand b ON b.id = m.brand_id
    WHERE b.name = ${literal(model.brand)} AND m.name = ${literal(model.name)}
      AND (SELECT run FROM _competitor_import_guard)
    ON CONFLICT DO NOTHING;`,
    `UPDATE catalog.vehicle_configuration
      SET identity_status='RESOLVED_FROM_PRIMARY_SOURCE',
          identity_evidence_id=${literal(evidenceId(identity))}::uuid,
          identity_note=${literal(identity.note)}
      WHERE id=${lookup} AND (SELECT run FROM _competitor_import_guard);`,
  ];
  ...
  for (const specification of configuration.specifications ?? []) {
    ...
    const assertionId = uuid(
      `assertion:${manifest.version}:${model.brand}:${model.name}:${configuration.name}:${configuration.market}:${configuration.modelYear}:${specification.attribute}:${JSON.stringify(specification.value ?? specification.availability)}:${specification.source}`,
    );
    ...
    statements.push(
      ...
      SELECT ${literal(assertionId)}::uuid, ${lookup}, d.id, ${literal(attribute.valueType)},
        ${value}, ${availability}, ${json(specification.qualifiers ?? {})},
        ${literal(specification.rawValue ?? String(specification.value ?? specification.availability))},
        'VERIFIED'
      FROM catalog.attribute_definition d
      WHERE d.code=${literal(attribute.code)} AND (SELECT run FROM _competitor_import_guard)
      ON CONFLICT (id) DO NOTHING;`,
      `INSERT INTO catalog.assertion_evidence (assertion_id,evidence_id)
      SELECT ${literal(assertionId)}::uuid, ${literal(evidenceId(evidence))}::uuid
      WHERE (SELECT run FROM _competitor_import_guard)
      ON CONFLICT DO NOTHING;`,
      `INSERT INTO catalog.accepted_specification
        (configuration_id,attribute_id,knowledge_status,assertion_id,reason)
      SELECT ${lookup}, d.id, 'KNOWN', ${literal(assertionId)}::uuid,
        'Verified against the cited current official Brazilian manufacturer source.'
      FROM catalog.attribute_definition d
      WHERE d.code=${literal(attribute.code)} AND (SELECT run FROM _competitor_import_guard)
      ON CONFLICT (configuration_id,attribute_id) DO UPDATE SET
...
function generateSql() {
  ...
  const lines = [
    'BEGIN;',
    'CREATE TEMP TABLE _competitor_import_guard(run boolean NOT NULL) ON COMMIT DROP;',
    `INSERT INTO _competitor_import_guard VALUES (NOT EXISTS (
    ...
    `DO $check$ BEGIN
      IF EXISTS (SELECT 1 FROM catalog.seed_dataset WHERE version=${literal(manifest.version)} AND sha256<>${literal(digest)}) THEN
        RAISE EXCEPTION 'Competitor dataset version exists with a different checksum';
      END IF;
    END $check$;`,
    ...manifest.sources.map(sourceSql),
    ...manifest.brands.map(
      (brand) => `INSERT INTO catalog.brand (id,name)
      SELECT ${literal(uuid(`brand:${brand.name}`))}::uuid, ${literal(brand.name)}
      WHERE (SELECT run FROM _competitor_import_guard)
      ON CONFLICT (name) DO NOTHING;`,
    ),
    ...models.map(
      (model) => `INSERT INTO catalog.vehicle_model (id,brand_id,name)
      SELECT ${literal(uuid(`model:${model.brand}:${model.name}`))}::uuid, b.id,
        ${literal(model.name)} FROM catalog.brand b
      WHERE b.name=${literal(model.brand)} AND (SELECT run FROM _competitor_import_guard)
    ...
    `DO $attributes$ BEGIN
      IF EXISTS (
        ...
        RAISE EXCEPTION 'Competitor dataset conflicts with an existing attribute definition';
      END IF;
    END $attributes$;`,
    ...
    ...manifest.supersessions.map((supersession) => {
      const target = {
        name: supersession.target.name,
        market: supersession.target.market,
        modelYear: supersession.target.modelYear,
      };
      return `UPDATE catalog.vehicle_configuration
        SET superseded_by=${configurationLookup(
          supersession.target.brand,
          supersession.target.model,
          target,
        )}
        WHERE id=${literal(supersession.configurationId)}::uuid
          AND superseded_by IS NULL AND (SELECT run FROM _competitor_import_guard);`;
    }),
    ...
    WHERE b.name=ANY(${literal(`{${manifest.brands.map((brand) => brand.name).join(',')}}`)}::text[])
...
if (process.argv[2] !== 'sql') {
  throw new Error(
    'Usage: node tools/catalog/competitors-brasil-2026/import.mjs sql',
  );
}
